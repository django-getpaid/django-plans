...
class InvoiceProformaManager(models.Manager):
    def get_query_set(self):
        return super(InvoiceProformaManager, self).get_query_set() \
                                                  .filter(type=AbstractInvoice.INVOICE_TYPES['PROFORMA'])


class InvoiceDuplicateManager(models.Manager):
    def get_query_set(self):
        return super(InvoiceDuplicateManager, self).get_query_set() \
    ...
    def clean(self):
        if self.number is None:
            invoice_counter_reset = getattr(
                settings, 'PLANS_INVOICE_COUNTER_RESET', self.NUMBERING.MONTHLY)

            Invoice = self.get_concrete_model()

            if invoice_counter_reset == self.NUMBERING.DAILY:
