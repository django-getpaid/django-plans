...
urlpatterns = [
    ...
    url(r'^order/extend/new/(?P<pk>[0-9a-f-]+)/$', CreateOrderView.as_view(), name='create_order_plan'),
    url(r'^order/upgrade/new/(?P<pk>[0-9a-f-]+)/$', CreateOrderPlanChangeView.as_view(), name='create_order_plan_change'),
    url(r'^change/(?P<pk>[0-9a-f-]+)/$', ChangePlanView.as_view(), name='change_plan'),
    url(r'^order/$', OrderListView.as_view(), name='order_list'),
    url(r'^order/(?P<pk>[0-9a-f-]+)/$', OrderView.as_view(), name='order'),
    url(r'^order/(?P<pk>[0-9a-f-]+)/payment/success/$', OrderPaymentReturnView.as_view(status='success'),
        name='order_payment_success'),
    url(r'^order/(?P<pk>[0-9a-f-]+)/payment/failure/$', OrderPaymentReturnView.as_view(status='failure'),
    ...
    url(r'^invoice/(?P<pk>[0-9a-f-]+)/preview/html/$', InvoiceDetailView.as_view(), name='invoice_preview_html'),
]

if getattr(settings, 'DEBUG', False):
    urlpatterns += [
        url(r'^fakepayments/(?P<pk>[0-9a-f-]+)/$', FakePaymentsView.as_view(), name='fake_payments'),
    ]
