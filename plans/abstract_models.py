...
class AbstractPlan(OrderedModel):
    ...
    def get_quota_dict(self):
        quota_dic = {}
        for plan_quota in get_model('plans', 'PlanQuota').objects.filter(plan=self).select_related('quota'):
            quota_dic[plan_quota.quota.codename] = plan_quota.value
        return quota_dic


class AbstractBillingInfo(models.Model):
    """
...
class AbstractQuota(OrderedModel):
...
class AbstractPlanPricing(models.Model):
...
class AbstractPlanQuota(models.Model):
    plan = models.ForeignKey('Plan')
...
class AbstractInvoice(models.Model):
    ...
    def clean(self):
        ...
        super(AbstractInvoice, self).clean()
...
#            pass
    ...
    def create(cls, order, invoice_type):
        ...
        if invoice_type == AbstractInvoice.INVOICE_TYPES['PROFORMA']:
