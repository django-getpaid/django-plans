...
In django-plans there is a common validation mechanism which requires defining ``PLAN_ACTIVATION_VALIDATORS`` variable in ``settings.py``.

The format of ``PLAN_ACTIVATION_VALIDATORS`` variable is given as a dict::
